...
class EDMAP(object):
    def __init__(self, lr=1, rho=0.3, channel=2, alpha=1e-6, numCol=0):
    ...
    def learningMAP(self, x, label):
        ...
        if len(self.w) < self.label.size:       # self.w.shape[0] < self.label.shape[0]
            ...
            if len(self.w) < label_ind + 1:
                ...
                self.w[label_ind] = self.X      # new column is added
            ...
            for i in range(self.numCell[label_ind]):
                ...
                for j in range(self.channel):
                    m = np.sum(fuzzy_and(self.w[label_ind][ind][j][0], self.X[j]) / np.sum(self.X[j]))
                    if m >= self.rho:
                        resonance += 1
                    else:
                        break
                if resonance == self.channel:
                    for j in range(self.channel):
                        self.w[label_ind][ind][j] = (1-self.lr) * self.w[label_ind][ind][j] + self.lr * fuzzy_and(self.X[j], self.w[label_ind][ind][j][0])
                    break
    ...
    def activateCell(self, label):
        ...
        for i in range(self.w[label].shape[0]):
            for k in range(self.channel):
                act_k[k] = self.gamma[k] * (np.sum(fuzzy_and(self.X[k], self.w[label][i][k][0])) / (self.alpha + np.sum(self.w[label][i][k])))
